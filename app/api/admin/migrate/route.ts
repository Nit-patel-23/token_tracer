...
async function localBackfillResearchAnalytics(limit?: number, offset?: number): Promise<{ processed: number }> {
  ...
  if (limit !== undefined) {
    ...
    const parallelChunkSize = 10;
    for (let cIdx = 0; cIdx < rows.length; cIdx += parallelChunkSize) {
      const chunk = rows.slice(cIdx, cIdx + parallelChunkSize);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Run outcomes / research data backfill
    const backfillStats = await localBackfillResearchAnalytics();

    return NextResponse.json({
      ok: true,
      message: `Migration complete. Backfilled ${backfillStats.processed} sessions.`
    });
  } catch (err: any) {
